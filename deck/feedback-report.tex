...
included (\repo{scripts/_probe-audit-hashes.mjs}{\_probe-audit-hashes.mjs}). Documentation claims are checked against XLS-65, XLS-66 and
...
\repro
\begin{steps}
\item \tm{LoanSet} with \tm{PaymentInterval} 60 and \tm{GracePeriod} 60. \tm{LoanManage} \tm{tfLoanImpair} returns \tm{tecTOO\_SOON} 31 s and 13 s before \tm{NextPaymentDueDate}, and \tm{tesSUCCESS} 5 s after.
...
\repro
\begin{steps}
\item \tm{LoanSet} with \tm{PaymentInterval} 90, \tm{GracePeriod} 60, \tm{PaymentTotal} 4 (\repo{scripts/_probe-pay-states.mjs}{\_probe-pay-states.mjs}).
\item 10 s after the due date, \tm{LoanPay} of \tm{ceil(PeriodicPayment) + LoanServiceFee + LatePaymentFee} returns \tm{tecEXPIRED} with \tm{Flags: 0}, and \tm{tesSUCCESS} with \tm{Flags: 262144}.
\item With \tm{LateInterestRate} 30\,\% (\repo{scripts/_probe-h2-race.mjs}{\_probe-h2-race.mjs}), the same amount returns \tm{tecINSUFFICIENT\_PAYMENT}. Sending 2\,765\,001 drops succeeds and takes 765\,003.
\end{steps}
...
\repro
\begin{steps}
\item \repo{scripts/h1-default.mjs}{node scripts/h1-default.mjs}, after \tm{setup-accounts.mjs}: a 200 XRP vault, 50 XRP of cover, one 50 XRP loan, defaulted after \tm{GracePeriod}.
...
\repro
\begin{steps}
\item \repo{scripts/_probe-cliffs.mjs}{node scripts/\_probe-cliffs.mjs}: a \tm{LoanSet} above the cover limit with ample liquidity, then one above the liquidity with ample cover. Both return \tm{tecINSUFFICIENT\_FUNDS}.
\end{steps}
...
The V1.1 documentation restricts \tm{LoanBrokerSet} on open-ended vaults, a rule added by
...
\item \tm{LoanBrokerSet} on a \tm{VaultKind: 1} vault returns \tm{tesSUCCESS} (\repo{scripts/_probe-h13-closed.mjs}{\_probe-h13-closed.mjs}).
\end{steps}
...
\tm{lsfMPTCanTrade} lets holders trade ``\emph{using the XRP Ledger DEX or AMM}''. The devnet has
no MPT trading amendment, only \tm{MPTokensV1} and \tm{DynamicMPT}.

\repro
\begin{steps}
\item \tm{vault\_info}: \tm{shares.Flags} is 56, that is 0x08, 0x10 and 0x20.
\item \tm{OfferCreate} by a holder, \tm{TakerGets} the share MPT, \tm{TakerPays} XRP: \tm{temDISABLED}, a local rejection with no hash (\repo{scripts/_probe-shares.mjs}{\_probe-shares.mjs}).
\end{steps}
