...
% Build : tectonic -X compile deck/feedback-report.tex --outdir deck && mv deck/feedback-report.pdf FEEDBACK.pdf
...
% correction proposée : dernier paragraphe de chaque constat
\newcommand{\fix}[1]{\par\addvspace{1pt}{\color{navy}\bfseries Proposed fix.}\enspace #1\par}
...
\hs{\texttt{xrpl@4.6.0}}, for every finding · the 15 XLS-65/66 transaction types are typed · no raw JSON · one helper replaced (finding 1) &
...
\begin{tabularx}{\linewidth}{@{}c >{\raggedright\arraybackslash}X l l l@{}}
\rowcolor{card}
{\bfseries\color{navy}\#} & {\bfseries\color{navy}Finding} & {\bfseries\color{navy}Category} & {\bfseries\color{navy}Severity} & {\bfseries\color{navy}Library}\\
\midrule
1 & \tm{fixCleanup3\_4\_0} changes impairment timing and the \tm{LoanSet} counterparty signature. xrpl.org does not list it, and the \tm{xrpl.js} signing helper predates it. & client libraries & \hs{High} & \tm{xrpl@4.6.0}\\
\rowcolor{card}
2 & A late \tm{LoanPay} without \tm{tfLoanLatePayment} returns \tm{tecEXPIRED}. The xrpl.org \tm{LoanPay} reference does not list this failure. & documentation/tutorials & \hs{High} & \tm{xrpl@4.6.0}\\
3 & The first-loss capital covered 0.5\,\% of a defaulted loan. The broker then withdrew the remaining cover. & UX & \hs{High} & \tm{xrpl@4.6.0}\\
\rowcolor{card}
4 & xrpl.org omits the sole-holder exception to share value. \tm{LossUnrealized} is absent from the node while zero. & documentation/tutorials & Medium & \tm{xrpl@4.6.0}\\
5 & \tm{loan\_info} and \tm{loan\_broker\_info} do not exist. \tm{mpt\_holders} needs Clio, which the devnet does not expose. & missing primitive & Medium & \tm{xrpl@4.6.0}\\
\rowcolor{card}
6 & Four result codes each cover situations that call for different actions. & UX & Medium & \tm{xrpl@4.6.0}\\
7 & The hackathon build reverts the V1.1 \tm{LoanBrokerSet} restriction. The event brief does not mention it. & documentation/tutorials & Medium & \tm{xrpl@4.6.0}\\
\rowcolor{card}
8 & The devnet listens only on ports 51233 and 51234. A filtered port and a stalled node show the same signature. & other & Medium & \tm{xrpl@4.6.0}\\
9 & Closing a vault to deposits requires setting \tm{AssetsMaximum} to \tm{AssetsTotal}. & missing primitive & Low & \tm{xrpl@4.6.0}\\
\rowcolor{card}
10 & Vault shares carry \tm{lsfMPTCanTrade}. \tm{OfferCreate} with a share amount returns \tm{temDISABLED}. & documentation/tutorials & Medium & \tm{xrpl@4.6.0}\\
\end{tabularx}}
{\fontsize{6.3}{7.6}\selectfont\color{muted}\hs{Severity.} High: blocks a minimum bar step for any team, or misstates the risk a depositor carries. Medium: costs time or a wrong design decision, with a workaround. Low: documented elsewhere or cosmetic.\par}

\vspace{4pt}
\begin{multicols}{2}\RaggedRight

\hh{The six questions of the brief}
\begin{enumerate}[leftmargin=10pt,itemsep=1.7pt,topsep=2.4pt,parsep=0pt,label={\color{blue}\bfseries\arabic*}]
\item \hs{Vault, broker and loan.} Clear in the references: only the vault owner can submit \tm{LoanBrokerSet}, so owner and broker are one account. The minimum bar's drawdown has no transaction: \tm{LoanSet} moves the principal out of the vault.
\item \hs{First-loss parameters.} No. \tm{CoverRateLiquidation} applies to the minimum cover, not to the defaulted debt (finding 3).
\item \hs{Multi-party \tm{LoanSet}.} The broker signs, the borrower adds \tm{CounterpartySignature} and submits. Both wallets ran in one script. The \tm{xrpl@4.6.0} helper for the second signature is rejected on this devnet (finding 1).
\item \hs{SDK or raw JSON.} All 15 types are typed. No raw JSON. One helper replaced (finding 1).
\item \hs{Position value, utilisation, liquidity, yield.} Not from ledger objects: five calls and six values computed client-side (finding 5), fields absent while zero (finding 4).
\item \hs{Explorer and docs against the ledger.} The docs differ on findings 1, 2, 4 and 10. The explorer shows \tm{tfLoanLatePayment} in hex (finding 2) and no \tm{Vault} field values in its \emph{Detailed} tab (finding 4).
\end{enumerate}
\ev{drawdown in LoanSet D19CD59FB93CCF28547533C76CC1AFA71CF592FD0EC9D0A9E217A9F6EAAD85DA}
...
\fix{List \tm{fixCleanup3\_4\_0} on \emph{Known Amendments} with its two lending changes. Update \emph{Manage a Loan}: impairment is accepted once \tm{NextPaymentDueDate} has passed. In \tm{xrpl.js}, sign \tm{CounterpartySignature} with \tm{encodeForSigningCounterparty}, with the previous prefix as an option for networks without the amendment.}
...
\fix{Serve JSON-RPC and WebSocket on port 443. Fund the \tm{destination} account from the faucet instead of creating a new one.}
...
\fix{Add a \tm{VaultSet} flag that closes a vault to deposits. Repeat ``\tm{0} means no cap'' on the \tm{VaultSet} reference.}
...
% ───────────── fin ─────────────
\hh{Minimum bar, guardrails, escrow swap}
The minimum bar run, the six guardrails, the closed-ended vault control and the escrow swap are
listed with explorer links in the README. All 15 XLS-65/66 transaction types were submitted at
least once, including a full \tm{tfLoanImpair} then \tm{tfLoanDefault} cycle.

\hh{What worked, limits, material}
\tm{tecHAS\_OBLIGATIONS} names its cause on the three delete transactions. \tm{VaultDelete} removes
third-party \tm{MPToken} objects and refunds their reserve. The cover floor is enforced to the drop,
and cover rates return \tm{temINVALID} when changed on an existing broker. The closed-ended vault
phases behave as XLS-65 describes. Limits: XRP only, so neither clawback was tested with an issued
asset; payment schedules of 60 to 90 seconds; one build. In the repo: \tm{scripts/raw-submit.mjs}
(counterparty signer), \tm{scripts/\_probe-audit-hashes.mjs} (hash audit),
\tm{deck/capture-explorer.mjs} (these screenshots). In passing: the Lending Protocol concept page
spells \tm{PrincipleOutstanding} and \tm{depostitor}; \tm{validateVaultCreate} in \tm{xrpl@4.6.0}
accepts any number for \tm{WithdrawalPolicy}, and 0, 2, 3, 99 and 255 return \tm{temMALFORMED}.
