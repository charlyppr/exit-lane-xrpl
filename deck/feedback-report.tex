...
\makeatletter\renewcommand\normalsize{\@setfontsize\normalsize{9.6}{11.6}}\makeatother
...
% ── briques ──
\newcommand{\hh}[1]{\par\addvspace{5pt}{\raggedright\hyphenpenalty=10000\exhyphenpenalty=10000\hdfont\color{navy}\fontsize{10.4}{12.2}\selectfont\bfseries #1\par}\nobreak\addvspace{1.5pt}}
...
\newcommand{\ev}[1]{\par{\raggedright\ttfamily\fontsize{5.3}{6.6}\selectfont\color{muted}#1\par}}
...
% utile de la carte. Le titre trop long pour 460 px est ramené à la largeur.
...
\setlist[itemize]{leftmargin=8pt,itemsep=1.2pt,topsep=1.5pt,parsep=0pt,label={\color{blue}\textbullet}}
...
\vspace{3pt}
{\renewcommand{\arraystretch}{1.1}\setlength{\tabcolsep}{3.5pt}\fontsize{7}{8.4}\selectfont
...
\begin{screen}
\shot{f1b-type}\shot{f1b-flags}
\end{screen}
\capt{Explorer. Same loan, same amount, with \texttt{tfLoanLatePayment}.}
...
\begin{screen}
\shot{f3a-type}\shot{f3a-flags}\shot{f3a-meta}
\end{screen}
\capt{Explorer. The default moves 0.02 XRP from the broker's pseudo-account (\texttt{rbhF8…}) to the vault's (\texttt{rsR4Q…}).}
\ev{923F7D616B094C82197506CF2867907A93067041E3990D24BD07A19035F73EAC}
\begin{screen}
\shotw{f3b-type}\shot{f3b-amount}\shot{f3b-date}
\end{screen}
\capt{Explorer. The broker withdraws the remaining cover 21 s after the default.}
...
\begin{screen}
\shot{f2-type}\shot{f2-meta}\vspace{3pt}\shot{f2-raw}
\end{screen}
\capt{Explorer. \emph{Detailed} tab, then \emph{Raw} tab with the \texttt{Vault} node expanded.}
...
\tm{unknownCmd}. A depositor's view takes five calls:
...
\begin{screen}
\shot{f5a-type}\shot{f5a-max}\shot{f5a-date}
\end{screen}
\capt{Explorer. \texttt{AssetsMaximum} set to 0 on a vault capped at 5 XRP and holding 3.}
\ev{A336D71E738AA27C2B449DE05A0FE5C8945733A53420B9B4D8390172404CC1E2}
\begin{screen}
\shot{f5b-type}\shot{f5b-amount}\shot{f5b-date}
\end{screen}
\capt{Explorer. Two seconds later, a 4 XRP deposit brings the same vault to 7 XRP.}
