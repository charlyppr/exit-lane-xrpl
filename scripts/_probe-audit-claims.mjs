// Audit de vérité des claims chiffrés de FEEDBACK.md : relit sur le ledger les
// nœuds derrière chaque affirmation et recalcule les formules. Lecture seule.
...
console.log("### FINDING 2 — prix de part / LossUnrealized (impair C8678C1C)");
for (const n of await nodesOf("C8678C1C0A10BE889124F40C03017038654B278E2D464FAC54784F55B35318FC")) {
  if (n.type !== "Vault") continue;
  console.log("   Vault au moment de l'impair:", JSON.stringify(n.final));
  const v = await idx(n.index).catch(() => null);
  if (v) console.log("   Vault aujourd'hui:", JSON.stringify({ AssetsTotal: v.AssetsTotal, LossUnrealized: v.LossUnrealized, ShareMPTID: v.ShareMPTID }));
  ...
  console.log(`   AssetsTotal ${at} · LossUnrealized ${lu} · net ${at - lu} · ratio net/brut = ${Number(at - lu) / Number(at)}`);
}

console.log("\n### FINDING 3 — paramètres des brokers des deux défauts");
for (const [tag, h] of [["A (vault 200 XRP)", "91F458E3E6244E1C3005345C5C83CDFAF0E8A1AF6E5587E100C302CA56743BC7"], ["B (vault 5 XRP)", "923F7D616B094C82197506CF2867907A93067041E3990D24BD07A19035F73EAC"]]) {
  ...
  console.log(` ${tag}`);
  if (!br) { console.log("   pas de LoanBroker"); continue; }
  ...
  console.log(`   dette avant défaut ${debt} · couverture prélevée ${coverTaken}`);
  console.log(`   debt × CoverRateMinimum/1e5 × CoverRateLiquidation/1e5 = ${(debt * cm * cl) / 10_000_000_000n} drops`);
  console.log(`   part de la dette absorbée = ${(Number(coverTaken) / Number(debt) * 100).toFixed(4)} %`);
}

console.log("\n### V1.1 — le vault de LoanBrokerSet 781F54B5 est-il open-ended ?");
