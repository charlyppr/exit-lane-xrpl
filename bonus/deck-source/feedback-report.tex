...
\hypersetup{pdftitle={Exit Lane: XRPL Lending Protocol Developer Feedback, CY-HACK},pdfauthor={CY-HACK}}
\tcbuselibrary{skins}
...
\definecolor{muted}{HTML}{6B7278}
\definecolor{fixtext}{HTML}{3A4047}
\definecolor{code}{HTML}{2B4A85}
...
\newfontfamily\hdfont{Poppins}[BoldFont={Poppins SemiBold},Ligatures=TeX]
...
\fancyfoot[L]{\scriptsize\color{muted}Exit Lane · CY-HACK · Developer feedback}
...
\newcommand{\tm}[1]{\leavevmode{\ttfamily\color{code}#1}}
...
\newcommand{\repo}[2]{\leavevmode\href{https://github.com/charlyppr/exit-lane-xrpl/blob/main/#1}{{\ttfamily\color{blue}#2}}}

% sévérité : seule pastille et seule icône du document
\newlength{\pillw}
\AtBeginDocument{\settowidth{\pillw}{\fontsize{6.4}{7}\selectfont\faExclamationCircle\hspace{2.6pt}\bfseries MEDIUM}}
\newcommand{\pill}[4]{\tcbox[on line,colback=#1,colframe=#1,boxrule=0pt,boxsep=0pt,left=3.2pt,right=3.2pt,
  top=1.5pt,bottom=1.5pt,arc=1.6pt]{\makebox[\pillw][c]{\normalfont\fontsize{6.4}{7}\selectfont\color{#2}#3\hspace{2.6pt}\bfseries #4}}}
...
% \clubpenalties : les 4 premières lignes du texte qui suit la carte restent collées à
% elle, donc une carte ne finit jamais seule en bas de colonne (\needspace ne voit pas la
% hauteur de colonne dans multicols). \repro rétablit la règle normale.
\newcommand{\finding}[4]{\par\addvspace{15pt}\noindent
  ...
  \begin{tcolorbox}[enhanced,colback=card,colframe=card,boxrule=0pt,arc=2pt,
    ...
    after={\par\nobreak\nointerlineskip\vskip\glueexpr 7pt-\parskip\relax},
    borderline west={2.6pt}{0pt}{sevrule}]
  \hypertarget{f#1}{}\raggedright
  {\hangindent=13pt\hangafter=1\noindent\makebox[13pt][l]{\hdfont\color{blue}\fontsize{10}{12}\selectfont\bfseries #1}{\def\tm##1{{\ttfamily\mdseries\color{navy}##1}}\hdfont\color{navy}\fontsize{10}{12}\selectfont\bfseries #4}\par}
  ...
  \end{tcolorbox}\clubpenalties 5 10000 10000 10000 10000 0 }
...
\newcommand{\repro}{\par\clubpenalties 0 \addvspace{3pt}\lbl{Repro steps}\par\nobreak\vspace{0.5pt}\@afterheading}
\makeatother
...
\newcommand{\fix}[1]{\par\addvspace{5pt}\begin{tcolorbox}[colback=bluecard,colframe=bluecard,boxrule=0pt,arc=2pt,
  left=6pt,right=6pt,top=4.5pt,bottom=4.5pt,boxsep=0pt,before skip=1pt,after skip=0pt,coltext=fixtext]\raggedright
...
\newtcolorbox{screen}{enhanced,colback=black,colframe=black,boxrule=0pt,arc=2.4pt,
  left=3.5pt,right=3.5pt,top=3pt,bottom=3pt,boxsep=0pt,before skip=0pt,after skip=4pt}
...
{\hdfont\color{white}\fontsize{9.5}{11}\selectfont\bfseries Exit Lane}\\[2pt]
{\fontsize{6.8}{8.4}\selectfont\color{sky}CY-HACK · DevEx pseudonym \texttt{late-quail-92}\\
\href{https://github.com/charlyppr/exit-lane-xrpl}{\texttt{github.com/charlyppr/exit-lane-xrpl}}}
\end{minipage}
\end{tcolorbox}
...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.18\linewidth}@{\hspace{8pt}}>{\raggedright\arraybackslash}p{0.19\linewidth}@{\hspace{8pt}}>{\raggedright\arraybackslash}X@{\hspace{8pt}}>{\raggedright\arraybackslash}p{0.165\linewidth}@{}}
...
\vspace{8pt}
{\fontsize{7.4}{9.6}\selectfont\color{body}The 143 transactions cited across the deliverables (this report, the README and the raw log) are validated on the ledger, type and result code
...
\hyperlink{f4}{\color{blue}\bfseries 4} & \hyperlink{f4}{A share \tm{Payment} to a new holder returns \tm{tecNO\_AUTH}} & documentation/tutorials & \sevMedium\\
\hyperlink{f5}{\color{blue}\bfseries 5} & \hyperlink{f5}{Vault shares carry \tm{lsfMPTCanTrade}, but \tm{OfferCreate} rejects them} & documentation/tutorials & \sevMedium\\
\hyperlink{f6}{\color{blue}\bfseries 6} & \hyperlink{f6}{No read command for loans or brokers} & missing primitive & \sevMedium\\
\hyperlink{f7}{\color{blue}\bfseries 7} & \hyperlink{f7}{xrpl.org omits the sole-holder exception, and zero-valued fields are absent} & documentation/tutorials & \sevMedium\\
\hyperlink{f8}{\color{blue}\bfseries 8} & \hyperlink{f8}{One result code covers causes that need different actions} & UX & \sevMedium\\
\hyperlink{f9}{\color{blue}\bfseries 9} & \hyperlink{f9}{No flag closes a vault to deposits} & missing primitive & \sevLow\\
\bottomrule
\end{tabularx}\par}
\vspace{4pt}
{\noindent\makebox[\linewidth][s]{\fontsize{6.6}{8.4}\selectfont\color{muted}\hs{High}:\space blocks a minimum bar step, or misstates a depositor's risk.\hfill\hs{Medium}:\space costs time or a wrong design, with a workaround.\hfill\hs{Low}:\space cheap to work around, or cosmetic.}\par}

\vspace{4pt}
\begin{multicols}{2}\RaggedRight\hyphenpenalty=10000\exhyphenpenalty=10000
...
% colonne des intitulés à la largeur du plus long, puis 5pt de gouttière
\newlength{\qlabw}\settowidth{\qlabw}{\bfseries First-loss parameters}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{\qlabw}@{\hspace{5pt}}>{\raggedright\arraybackslash}X@{}}
...
\hs{Position and yield} & Computed client-side from five calls (\hyperlink{f6}{6}).\\
\hs{Docs and explorer} & The docs differ in \hyperlink{f1}{1}, \hyperlink{f2}{2}, \hyperlink{f4}{4}, \hyperlink{f5}{5}, \hyperlink{f7}{7}. The explorer hides a flag name (\hyperlink{f2}{2}) and \tm{Vault} fields (\hyperlink{f7}{7}).\\
\end{tabularx}}

% ───────────── 1 ─────────────
\finding{1}{High}{client libraries}{\tm{fixCleanup3\_4\_0} is enabled and missing from xrpl.org}

The amendment, new in \tm{rippled} 3.4.0-rc1, is active on the devnet and absent from
\emph{Known Amendments}. It changes two lending behaviours, and the tutorial reflects neither.
...
\hs{Counterparty signature.} Since rippled PR \#8162, \tm{CounterpartySignature} uses the
\tm{CPT} prefix under the amendment, the previous prefix without it. \tm{signLoanSetByCounterparty}
has no option: \tm{xrpl@4.6.0} always signs with the previous prefix, \tm{xrpl@5.2.0} (xrpl.js PR
\#3462) always with \tm{CPT}, which \tm{rippled} rejects on networks without the amendment.
...
\fix{List \tm{fixCleanup3\_4\_0} on \emph{Known Amendments} with both lending changes. Update \emph{Manage a Loan}: impairment is accepted once \tm{NextPaymentDueDate} has passed. In \tm{xrpl.js}, pick the counterparty prefix from the network's \tm{fixCleanup3\_4\_0} status, as \tm{rippled} does.}
...
% ───────────── 4 ─────────────
\finding{4}{Medium}{documentation/tutorials}{A share \tm{Payment} to a new holder returns \tm{tecNO\_AUTH}}

The Single Asset Vault page, \emph{Can a Depositor Transfer Shares to Another Account?}, lists
five failure causes, then: ``\emph{Otherwise, a new MPT entry is created for their account.}''
Without an \tm{MPToken} for the share issuance, the payee gets \tm{tecNO\_AUTH}, a cause the page
omits, until it submits \tm{MPTokenAuthorize}.

\repro
\begin{steps}
\item Public XRP vault, no freeze. A share \tm{Payment} to an account without \tm{MPToken}: \tm{tecNO\_AUTH} (\repo{bonus/scripts/_probe-shares2.mjs}{\_probe-shares2.mjs}).
\item The payee submits \tm{MPTokenAuthorize}, and the same \tm{Payment} returns \tm{tesSUCCESS} (\repo{bonus/scripts/_probe-shares3.mjs}{\_probe-shares3.mjs}).
\end{steps}

\txs refused~\tx{789AAF7E228CC085199E2FB38B07A10DFC68A45C76C5FC3EFDB498BC5FD512A4}, opt-in~\tx{3AD6F7E37DF8778E03B12279F9CE4F3FD25B1C0FEECA774920E49ED2894F3EA4}, accepted~\tx{B20699B9EBB2E3B60439FD86316CEC44F9579DBFABED6C779675522233791601}

\fix{Replace the section's last sentence: the payee needs an \tm{MPToken} (\tm{MPTokenAuthorize}), otherwise \tm{tecNO\_AUTH}.}

% ───────────── 5 ─────────────
\finding{5}{Medium}{documentation/tutorials}{Vault shares carry \tm{lsfMPTCanTrade}, but \tm{OfferCreate} rejects them}
...
\tm{lsfMPTCanTrade} lets holders trade ``\emph{using the XRP Ledger DEX or AMM}''. \tm{OfferCreate}
accepts MPTs only under \tm{MPTokensV2} (XLS-82), which \tm{rippled} \tm{develop} marks as not
supported.

\repro
\begin{steps}
\item \tm{vault\_info}: \tm{shares.Flags} 56, i.e. 0x08, 0x10, 0x20.
\item \tm{OfferCreate} selling shares for XRP: \tm{temDISABLED}, a local rejection with no hash (\repo{bonus/scripts/_probe-shares.mjs}{\_probe-shares.mjs}).
\end{steps}

\fix{State on the \tm{MPTokenIssuance} reference that DEX trading of MPTs needs \tm{MPTokensV2}, or leave \tm{lsfMPTCanTrade} unset on vault shares until it ships.}

% ───────────── 6 ─────────────
\finding{6}{Medium}{missing primitive}{No read command for loans or brokers}
...
amount, utilisation, broker debt capacity. \tm{mpt\_holders}, which lists a vault's depositors,
is documented as Clio-only.\par\nobreak
...
\repro
\begin{steps}
\item \tm{loan\_info} and \tm{loan\_broker\_info} return \tm{unknownCmd}. Neither has a documentation page.
\end{steps}

\fix{Add \tm{loan\_broker\_info} and \tm{loan\_info}, or list a vault's brokers and loans in \tm{vault\_info}. Return share value, withdrawable amount and utilisation. Serve \tm{mpt\_holders} from \tm{rippled}.}

% ───────────── 7 ─────────────
\finding{7}{Medium}{documentation/tutorials}{xrpl.org omits the sole-holder exception, and zero-valued fields are absent}
...
\tm{TotalValueOutstanding} at zero.
...
% ───────────── 8 ─────────────
\finding{8}{Medium}{UX}{One result code covers causes that need different actions}
...
\repro
\begin{steps}
\item \repo{bonus/scripts/_probe-cliffs.mjs}{\_probe-cliffs.mjs}: a \tm{LoanSet} above the cover limit, then one above the liquidity. Both return \tm{tecINSUFFICIENT\_FUNDS}.
\end{steps}
...
% ───────────── fin ─────────────
\hh{Minor issues\hspace{6pt}\raisebox{1pt}{\sevLow}}
...
\toprule
\tabhead{Issue} & \tabhead{Category}\\
\midrule
Typos: \tm{PrincipleOutstanding} and \tm{depostitor} on the Lending Protocol page, ``the the'' and \tm{oustanding} on \tm{LoanManage}. & documentation/tutorials\\
\tm{LoanManage} with no flag is an undocumented no-op (\tm{tesSUCCESS}). & documentation/tutorials\\
The lending pages never say that due dates are compared with the parent ledger close time, rounded to 10 s. & documentation/tutorials\\
\tm{validateVaultCreate} accepts any \tm{WithdrawalPolicy}; the ledger takes only 1. & client libraries\\
\tm{validateLoanManage} only rejects impair with unimpair; \tm{tfLoanDefault} combinations reach the ledger (\tm{temINVALID\_FLAG}). & client libraries\\
\tm{submitAndWait} throws on \tm{tem} with the code only in the message text, but returns \tm{tec} results. & client libraries\\
\bottomrule
\end{tabularx}}

\hh{What worked}
\tm{tecHAS\_OBLIGATIONS} names its cause on the three delete transactions. \tm{VaultDelete}
removes third-party \tm{MPToken} objects and refunds their reserve. The cover floor holds to the
drop, cover rates are immutable (\tm{temINVALID}), and closed-ended phases match XLS-65.

\hs{Limits.} XRP only. Intervals of 60 to 90 s. One build. No Clio, so \tm{mpt\_holders} untested.
\tm{xrpl@5.2.0} read in its source, not run. Explorer checked on \tm{custom.xrpl.org} only. Hashes
on the hackathon devnet, which may close; to replay, edit \tm{scripts/config.mjs}. Full runs in the
\href{https://github.com/charlyppr/exit-lane-xrpl\#readme}{\color{blue}README}.
