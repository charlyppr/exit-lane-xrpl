// H2: does the exact amount of a late payment move while we read it?
// Loan with a high LateInterestRate to make the drift visible.
...
while (n && Number(n.PaymentRemaining ?? 0) > 0) {
  ...
  const r = await safe(borrower.seed, { TransactionType: "LoanPay", LoanID: L, Amount: String(due(n)), ...(late ? { Flags: LATE } : {}) },
    `LoanPay remaining (${n.PaymentRemaining})`);
  if (!r.ok) break;
  n = await readEntry(c, L).catch(() => null);
}
