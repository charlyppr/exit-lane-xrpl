// Où va l'excédent d'un paiement en retard surpayé ? Lecture des métadonnées.
...
console.log(`LoanPay ${pay.hash}\n  Amount envoyé : ${t.Amount} drops (${fmt(t.Amount)})  Flags ${t.Flags}`);
console.log(`  Fee : ${t.Fee} drops`);
console.log("\n── mouvements de solde dans les métadonnées ──");
for (const n of pay.meta.AffectedNodes) {
  ...
  if (type === "AccountRoot") {
    ...
    console.log(`  Vault            AssetsTotal ${fmt(k.PreviousFields?.AssetsTotal ?? 0)} → ${fmt(k.FinalFields?.AssetsTotal ?? 0)} · dispo ${fmt(k.PreviousFields?.AssetsAvailable ?? 0)} → ${fmt(k.FinalFields?.AssetsAvailable ?? 0)}`);
  } else if (type === "Loan") {
    const p = k.PreviousFields ?? {}, f = k.FinalFields ?? {};
    console.log(`  Loan             principal ${fmt(p.PrincipalOutstanding ?? 0)} → ${fmt(f.PrincipalOutstanding ?? 0)} · total ${fmt(p.TotalValueOutstanding ?? 0)} → ${fmt(f.TotalValueOutstanding ?? 0)} · échéances ${p.PaymentRemaining ?? "?"} → ${f.PaymentRemaining ?? "?"}`);
    console.log(`                   NextPaymentDueDate ${p.NextPaymentDueDate ?? "?"} → ${f.NextPaymentDueDate ?? "?"}`);
  } else if (type === "LoanBroker") {
    console.log(`  LoanBroker       cover ${fmt(k.PreviousFields?.CoverAvailable ?? 0)} → ${fmt(k.FinalFields?.CoverAvailable ?? 0)} · dette ${fmt(k.PreviousFields?.DebtTotal ?? 0)} → ${fmt(k.FinalFields?.DebtTotal ?? 0)}`);
  }
}
