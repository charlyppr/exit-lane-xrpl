// Le vault 0F8196BF… vient d'être supprimé. Ses parts (MPTID 0000000143CE19F9265FA64343508B0E6130D5792A4B2780)
// étaient détenues par spare et borrower à solde nul. Que sont devenus ces objets ?
...
for (const n of ["spare", "borrower", "broker"]) {
  ...
  console.log(`${n.padEnd(9)} OwnerCount ${String(info.result.account_data.OwnerCount).padStart(3)} · objets ${String(r.result.account_objects.length).padStart(3)} · MPToken ${String(mpts.length).padStart(3)} · orphelin du vault supprimé : ${orphan ? "OUI ⚠️ " + JSON.stringify({ idx: orphan.index, amt: orphan.MPTAmount ?? 0, flags: orphan.Flags }) : "non"}`);
}

console.log("\n--- L'émission de parts existe-t-elle encore ? ---");
for (const [cmd, arg] of [["vault_info", { vault_id: VAULTID }], ["mpt_holders", { mpt_issuance_id: MPTID }]]) {
  ...
  } catch (e) { console.log(`${cmd} → ERREUR ${e.message} ${JSON.stringify(e.data ?? {}).slice(0, 200)}`); }
}
console.log("\n--- ledger_entry sur l'objet MPTokenIssuance ---");
try {
  const r = await c.request({ command: "ledger_entry", mpt_issuance: MPTID, ledger_index: "validated" });
  console.log("MPTokenIssuance ENCORE PRÉSENTE :", JSON.stringify(r.result.node));
